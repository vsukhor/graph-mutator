...
//template<typename Ch>
//void Graph<Ch>::
//update_adjacency_cmpt(const CmpId c) noexcept
//{
//    if (cpagl.size() != cmpt_num()) {
//        cpagl.resize(cmpt_num());
//        clvisited.resize(cmpt_num());
//    }
//    cpagl[c] = make_adjacency_list(c);
//}
...
make_indma() noexcept
{
    // Set 'cps', 'glc', 'glm' and 'gla':
    std::fill(cps.begin(), cps.end(), 0);    // component size, # of edges
    for (const auto& m: cn)
        cps[m.c] += m.length();

    glc.resize(cmpt_num());
    for (CmpId j=0; j<cmpt_num(); ++j)
        glc[j].resize(cps[j]);

    glm.resize(edgenum);
    gla.resize(edgenum);
    for (ChId j=0; j<chain_num(); ++j)
        for (szt k=0; k<cn[j].length(); ++k) {
            const auto& g = cn[j].g[k];
            glm[g.ind] = j;
            gla[g.ind] = k;
            ct[g.c].gl[g.indc] = g.ind;
        }
}
 */
...
template<typename Ch>
bool Graph<Ch>::
dfs(const ChId w1, const EndId e1,
    const ChId w2, const EndId e2)
{
    for (const auto& s : cn[w1].ngs[e1]()) {
        const auto n = s.w;
        const auto e = s.e;
        if (n == w2) {
            if (e == e2)
...
template<typename Ch>
void Graph<Ch>::
populate_component_vectors() noexcept
{
    for (auto& cc: ct)
        cc.clear();
    ...
    for (ChId j=0; j<chain_num(); ++j) {
        ...
        ct[c].ww.push_back(j);    // indexes componentwise

        const auto nnd2 = m.template num_vertices<2>();
        nn[1] += nnd2;
        ...
        if (m.has_one_free_end()) {
            ...
            const auto oe = Ch::opp_end(e);
            ++nn[0];
            if (m.ngs[oe].num() == 2) {
                ct[c].chis.cn13.push_back(EndSlot{j, e});
                chis.cn13.push_back(EndSlot{j, e});
                ++nn[2];
            }
            else if (m.ngs[oe].num() == 3) {
                ct[c].cn14.push_back(EndSlot{j, e});
                cn14.push_back(EndSlot{j, e});
                ++nn[3];
            }
            else
                assignment_impossble(j);
        }
        else if (nA == 0 && nB == 0) {
            ct[c].cn11 = j;  // having both ends free, it is a separate chain
            cn11.push_back(j);
            nn[0] += 2;
        }
        else if (m.is_disconnected_cycle()) {
            ct[c].cn22 = j;
            cn22.push_back(j);  // it is a separate chain as it has 2 free ends
        }
        else if (nA == 2 && nB == 2) {
            ct[c].cn33.push_back(j);
            cn33.push_back(j);
            nn[2] += 2;
        }
        else if (nA == 2 && nB == 3) {
            ct[c].cn34.push_back(EndSlot{j, Ends::A});
            cn34.push_back(EndSlot{j, Ends::A});
            ++nn[2];
            ++nn[3];
        }
        else if (nA == 3 && nB == 2) {
            ct[c].cn34.push_back(EndSlot{j, Ends::B});
            cn34.push_back(EndSlot{j, Ends::B});
            ++nn[2];
            ++nn[3];
        }
        else if (nA == 3 && nB == 3) {
            ct[c].cn44.push_back(j);
            cn44.push_back(j);
            nn[3]+= 2;
        }
        else
...
populate_component_vectors() noexcept
{
    ...
    {
        cn[j].print("assignment_impossble ");

        abort("Error in populate_component_vectors: ",
                  "failed classification for ", j);

    };

    cn11.clear();
    cnc11.resize(cmpt_num());
    std::fill(cnc11.begin(), cnc11.end(), undefined<ChId>);

    cn22.clear();
    cnc22.resize(cmpt_num());
    std::fill(cnc22.begin(), cnc22.end(), undefined<ChId>);

    cn33.clear();
    cnc33.resize(cmpt_num());
    for (auto& o : cnc33) o.clear();

    cn44.clear();
    cnc44.resize(cmpt_num());
    for (auto& o : cnc44) o.clear();

    cn13.clear();
    cnc13.resize(cmpt_num());
    for (auto& o : cnc13) o.clear();

    cn14.clear();
    cnc14.resize(cmpt_num());
    for (auto& o : cnc14) o.clear();

    cn34.clear();
    cnc34.resize(cmpt_num());
    for (auto& o : cnc34) o.clear();

    nn = {{}};

    cpcn.resize(cmpt_num());
    for (auto& o : cpcn) o.clear();    // # of chains

    for (ChId j=0; j<chain_num(); ++j) {
        const auto& m = cn[j];
        const auto c = m.c;
        cpcn[c].push_back(j);    // indexes componentwise
        const auto nnd2 = m.template num_vertices<2>();
        nn[1] += nnd2;
        ...
        if (m.has_one_free_end()) {
            ...
            ++nn[0];
            if (m.ngs[oe].num() == 2) {
                cnc13[c].push_back(EndSlot{j, e});
                cn13.push_back(EndSlot{j, e});
                ++nn[2];
            }
            else if (m.ngs[oe].num() == 3) {
                cnc14[c].push_back(EndSlot{j, e});
                cn14.push_back(EndSlot{j, e});
                ++nn[3];
            }
            else
            ...
            cnc11[c] = j;  // having both ends free, it is a separate chain
            cn11.push_back(j);
            nn[0] += 2;
        }
        else if (m.is_disconnected_cycle()) {
            ...
            cnc34[c].push_back(EndSlot{j, Ends::A});
            cn34.push_back(EndSlot{j, Ends::A});
            ++nn[2];
            ++nn[3];
        }
        else if (nA == 3 && nB == 2) {
            cnc34[c].push_back(EndSlot{j, Ends::B});
            cn34.push_back(EndSlot{j, Ends::B});
            ++nn[2];
            ++nn[3];
        }
